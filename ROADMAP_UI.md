...
## M53 — Surface the scenario-presets catalogue: GET /scenarios (SPEC §3/§27/§34)
The engine track's commit 29e51af added `GET /scenarios` (+ `GET /scenarios/{id}`):
a curated library of ready-to-run canonical demo policies. Every *other* endpoint
first requires the caller to author or compile a Policy DSL, so nothing advertised
the demo's own canonical scenarios for a judge (or the UI) to one-click load —
they had to know to type the flagship congestion-charge prompt from memory. This
is the twin's missing on-ramp: a menu you can browse, pick a lever from, and load
straight into the workspace so every downstream panel (Parliament, Simulate,
Compare, …) runs against it. Pure surfacing of real backend data — each card
embeds the **real** compiler output and the panel mints no numbers; loading a
scenario is exactly equivalent to typing its prompt and pressing Compile.
- [x] Add `getScenarios()` (`GET /scenarios`) plus `ScenarioCard` / `ScenarioLibrary` types to `lib/api.ts`, mirroring the honest throw-on-error contract (so the panel shows a waiting/error state with Retry, never an invented menu); each card reuses the existing `CompileResponse`/`PolicyDSL` types so the two schemas can't drift. Built `ScenariosPanel` (new "Scenarios" tab, registered in `PanelTabs` after Capabilities — the policy-independent, loads-on-mount family): renders the Observed provenance topline + note, a presentation-only family filter (compiler-derived family chips) with a `showing N/total` readout so a filter can never make the menu look smaller than it is, and one card per scenario. Each card shows the title, compiler-derived family chip, SPEC chips, summary, objective/constraint goal chips, and a **"Load into workspace"** button that publishes the card's *pre-compiled* DSL to the shared `TwinStore` (exactly as the drafting box does after `POST /policy/compile`), so every downstream panel lights up against it; the active scenario is matched by comparing the store's compiled policy to each card's DSL so the "✓ Loaded — active policy" badge stays honest regardless of how the policy got there. An expandable details block shows the natural-language prompt, the reviewable assumptions (field/value/source/confidence + compiler method), any warnings, and the full compiled DSL. Honesty (SPEC §34): the catalogue is Observed about itself, each card's compiled DSL is stamped **Generated** (structuring, not simulation), and an active-card note makes clear no numbers are minted here — the figures come downstream from the deterministic sim layers. Added `.scn-*` styles to `globals.css` (reuses `.reg-*` / `.tag.*` / `.btn`); README endpoint table gains the Scenarios row (plus the previously-missing Capabilities row). No new numeric model, no LLM on any path, no endpoints beyond the documented one. `tsc --noEmit` + `next build` + `next lint` + `npm test` (41) all clean (SPEC §3/§27/§28/§34)
